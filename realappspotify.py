...
def load_data():
    try:
        # Changez le chemin ici si nécessaire
        music = pd.read_csv('spotify-2023_2.csv')  
        if music.empty:
            st.error("Le fichier CSV est vide. Veuillez vérifier le fichier.")
        return music
    except FileNotFoundError:
        st.error("Le fichier CSV n'a pas été trouvé. Veuillez vérifier le chemin.")
    except pd.errors.EmptyDataError:
        st.error("Le fichier CSV est vide ou mal formé.")
...
def recommend(song):
    try:
        ...
        return (recommended_music_names, recommended_music_posters, recommended_music_urls, 
                recommended_youtube_urls, recommended_titles, recommended_views)
    except IndexError:
